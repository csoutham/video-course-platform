<x-public-layout>
    ...
    <div class="space-y-6">
        ...
        @if ($gifts->isEmpty())
            <div class="vc-panel border-dashed p-6 text-sm text-slate-600">You have not sent any gifts yet.</div>
        @else
            <div class="space-y-3">
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <div class="space-y-1">
                                <p class="text-sm font-semibold text-slate-900">
                                    {{ $gift->course->title ?? 'Course #'.$gift->course_id }}
                                </p>
                                <p class="text-xs text-slate-500">Recipient: {{ $gift->recipient_email }}</p>
                                <p class="text-xs text-slate-500">
                                    Status:
                                    <span class="font-semibold">{{ ucfirst($gift->status) }}</span>
                                </p>
                            </div>
